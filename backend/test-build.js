...
async function runTest() {
    ...
    const response = await fetch('http://localhost:5001/api/pwa/build', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            user_id: 'test_agent_' + Math.random().toString(36).substring(7),
    ...
    while (status === 'running') {
        ...
        const statusRes = await fetch(`http://localhost:5001/api/pwa/build/status/${buildId}`);
        ...
        if (status === 'success') {
            console.log("\n=== BUILD SUCCESSFUL ===");
            console.log(`Download URL: http://localhost:5001${data.apkUrl}`);
            break;
        } else if (status === 'failed') {
            console.error("\n=== BUILD FAILED ===");
            console.error(`Error: ${data.error}`);
            const logsRes = await fetch(`http://localhost:5001/api/pwa/build/logs/${buildId}`);
