...
async function runPwaPackagePipeline(buildId, { userId, websiteUrl, appName, shortName, themeColor, backgroundColor, sourceType, htmlContent, iconUrl, cacheStrategy, workspaceDir, buildsDir, plan, androidBuildFormat }) {
    ...
    try {
        ...
        if (sourceType === 'html') {
            ...
            logMsg(buildId, `Setting WebView source URL wrapper: ${websiteUrl}`);
            // Place placeholder index.html so Capacitor sync works
            fs.writeFileSync(path.join(wwwDir, 'index.html'), `<html><head><meta http-equiv="refresh" content="0;url=${websiteUrl}"></head><body>Redirecting to ${websiteUrl}</body></html>`);
        }
    ...
    cleartext: true
        ...
        fs.writeFileSync(localPropsPath, `sdk.dir=C:\\\\Users\\\\MBiG\\\\AppData\\\\Local\\\\Android\\\\Sdk\n`, 'utf8');
