...
class MockQueryBuilder {
  ...
  private save() {
    if (typeof window !== 'undefined') {
      try {
        ...
        console.warn(`localStorage quota exceeded for mock_db_${this.tableName}, keeping in memory only`);
      }
    }
  }
  ...
  private executeMutationAndGetResults() {
    ...
    if (this.pendingUpdate) {
      if (this.tableName === 'profiles') {
        ...
        if (user && user.email === 'ritikjagnit@gmail.com') {
          this.pendingUpdate.plan = 'pro';
        }
      }
    ...
    if (this.pendingUpsert) {
      ...
      for (const item of items) {
        if (this.tableName === 'profiles') {
          const user = firebaseAuth.currentUser;
          if (user && user.email === 'ritikjagnit@gmail.com' && (item.id === user.uid || item.email === 'ritikjagnit@gmail.com')) {
            item.plan = 'pro';
          }
        }
    ...
    if (this.tableName === 'profiles') {
      ...
      if (user && user.email === 'ritikjagnit@gmail.com') {
        results = results.map(item => {
          if (item.id === user.uid || item.email === 'ritikjagnit@gmail.com') {
            return { ...item, plan: 'pro' };
          }
          return item;
        });
      }
  ...
  async maybeSingle() {
    ...
    if (this.tableName === 'user_roles') {
      const user = firebaseAuth.currentUser;
      if (user && (user.email === 'ritik@gmail.com' || user.email === 'ritikjagnit@gmail.com' || user.email === 'jahanpatle01@gmail.com')) {
    ...
    return {
      upload: async (path: string, file: File | Blob) => {
        try {
          ...
          if (typeof window !== 'undefined') {
            memoryStorage.set(`mock_storage_${path}`, dataUrl);
            try {
              localStorage.setItem(`mock_storage_${path}`, dataUrl);
            } catch (e) {
              console.warn(`localStorage quota exceeded for mock_storage_${path}, keeping in memory only`);
            }
          }
      ...
      getPublicUrl: (path: string) => {
        let dataUrl = memoryStorage.get(`mock_storage_${path}`);
        if (!dataUrl && typeof window !== 'undefined') {
          dataUrl = localStorage.getItem(`mock_storage_${path}`) || "";
        }
        return {
          data: {
            publicUrl: dataUrl || ""
